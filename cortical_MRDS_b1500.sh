...
dwi=${SUBJECTS_DIR}/${sID}/dwi_b1500/dwi_subset_maxb-1500.nii.gz
bvec=${SUBJECTS_DIR}/${sID}/dwi_b1500/dwi_subset_maxb-1500.bvec
bval=${SUBJECTS_DIR}/${sID}/dwi_b1500/dwi_subset_maxb-1500.bval
scheme=${SUBJECTS_DIR}/${sID}/dwi_b1500/dwi_subset_maxb-1500.scheme
outbase=${SUBJECTS_DIR}/${sID}/dwi_b1500/${sID}
nVoxPerJob=10000
scratch_dir=${SUBJECTS_DIR}/${sID}/dwi_b1500/tmp
...
then
  ...
  mask=${SUBJECTS_DIR}/${sID}/dwi_b1500/mask.nii.gz
...
doFixels=1
fcheck=${SUBJECTS_DIR}/${sID}/dwi_b1500/mrds_fixels/index.mif
echolor green "[INFO] Looking for file: $fcheck"
if [ -f $fcheck ]
then
  echolor yellow "[INFO] File found $fcheck"
...
if [ $doFixels -eq 1 ]
then
   mkdir -pv ${SUBJECTS_DIR}/${sID}/dwi_b1500/mrds_fixels
   for v in FA MD COMP_SIZE
   do
   tmpDir=$(mktemp -u)
    my_do_cmd inb_mrds_scalePDDs.sh \
        -e 0.0000000000000001 \
        ${SUBJECTS_DIR}/${sID}/dwi_b1500/${sID}_MRDS_Diff_BIC_PDDs_CARTESIAN.nii.gz \
        ${SUBJECTS_DIR}/${sID}/dwi_b1500/${sID}_MRDS_Diff_BIC_${v}.nii.gz \
        ${SUBJECTS_DIR}/${sID}/dwi_b1500/${sID}_MRDS_Diff_BIC_PDDs_CARTESIAN_scaled-by-${v}.nii.gz

    my_do_cmd peaks2fixel \
        ${SUBJECTS_DIR}/${sID}/dwi_b1500/${sID}_MRDS_Diff_BIC_PDDs_CARTESIAN_scaled-by-${v}.nii.gz \
        $tmpDir
    mv -v ${tmpDir}/amplitudes.mif \
        ${SUBJECTS_DIR}/${sID}/dwi_b1500/mrds_fixels/MRDS_DIFF_BIC_${v}.mif
    mv -v ${tmpDir}/{directions,index}.mif ${SUBJECTS_DIR}/${sID}/dwi_b1500/mrds_fixels/
    rm -fR $tmpDir
    done
fi
...
if [ -f $roi ]; then rm $roi;fi
    ...
    # my_do_cmd peaks2fixel \
    #     ${outbase}_MRDS_Diff_BIC_PDDs_CARTESIAN_scaled.nii.gz \
    #     ${SUBJECTS_DIR}/${sID}/dwi_b1500/mrds_fixels
